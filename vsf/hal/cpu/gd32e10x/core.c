...
vsf_err_t vsfhal_core_init(void *p)
{
	...
	if (vsfhal_info.clk_enable & GD32E10X_CLK_HSE)
	{
	...
	if (vsfhal_info.clk_enable & GD32E10X_CLK_PLL)
	{
		if (vsfhal_info.pllsrc == GD32E10X_PLLSRC_HSI8M_D2)
		{
			RCU_CFG0 &= ~RCU_CFG0_PLLSEL;
		}
		else if (vsfhal_info.pllsrc == GD32E10X_PLLSRC_HSI48M)
		{
		...
		else if (vsfhal_info.pllsrc == GD32E10X_PLLSRC_HSE)
		{
		...
		RCU_CFG0 |= ((tmp32 & 0xf) << 18) | ((tmp32 & 0x1) ? RCU_CFG0_PLLMF_4 : 0);
	...
	if (vsfhal_info.clksrc == GD32E10X_CLKSRC_HSE)
		tmp32 = vsfhal_info.hse_freq_hz;
	else if (vsfhal_info.clksrc == GD32E10X_CLKSRC_PLL)
	...
	if (vsfhal_info.cpu_freq_hz > 90000000)
		FMC_WS = (FMC_WS & ~FMC_WS_WSCNT) | 3;
	else if (vsfhal_info.cpu_freq_hz > 60000000)
		FMC_WS = (FMC_WS & ~FMC_WS_WSCNT) | 2;
	else if (vsfhal_info.cpu_freq_hz > 30000000)
		FMC_WS = (FMC_WS & ~FMC_WS_WSCNT) | 1;
	else
		FMC_WS &= ~FMC_WS_WSCNT;
	...
	if (!(vsfhal_info.clk_enable & GD32E10X_CLK_HSI8M))
		RCU_CTL &= ~RCU_CTL_IRC8MEN;
...
//#if VSFHAL_USART3_ENABLE || VSFHAL_USART4_ENABLE
//	extern void gd32e10x_usart34_poll(void);
//	gd32e10x_usart34_poll();
//#endif
}
