...
#define EP_IN_TEMP_BUFFER_NUM		1
static uint8_t ep_in_buffer[EP_IN_TEMP_BUFFER_NUM][64];
static uint8_t ep_in_buffer_size[EP_IN_TEMP_BUFFER_NUM];
vsf_err_t vsfhal_usbd_init(int32_t int_priority)
{
	...
	USBD->BUSINTEN = USBD_BUSINTEN_RSTIEN_Msk | USBD_BUSINTEN_SOFIEN_Msk;
...
#else
		if (index < EP_IN_TEMP_BUFFER_NUM)
			ep_in_buffer_size[index] = min(size, 64);
#endif
		return VSFERR_NONE;
	}
	return VSFERR_BUG;
...
#else
		if (index < EP_IN_TEMP_BUFFER_NUM)
			memcpy(ep_in_buffer[index], buffer, min(size, 64));
#endif
		return VSFERR_NONE;
	}
	return VSFERR_BUG;
...
static uint8_t sof_tick, get_out = 0;
//#pragma optimize = low
void USB_Istr(void)
{
	...
	if (IrqStL & USBD_GINTSTS_USBIF_Msk) {
		...
		{
			USBD->BUSINTSTS = USBD_BUSINTSTS_SOFIF_Msk;
			sof_tick++;
			...
			if (sof_tick & 0x1)
			{
				...
				{		
					...
					{
						vsfhal_usbd_callback.on_out(vsfhal_usbd_callback.param, get_out);
					}
					get_out = 0;
				}
				for (i = 0; i < EP_IN_TEMP_BUFFER_NUM; i++)
				{
					if (ep_in_buffer_size[i])
					{
						for (j = 0; j < ep_in_buffer_size[i]; j++)
						{
							NUC505_USBD_EP_REG(i, EPDAT_BYTE) = ep_in_buffer[i][j];
						}
						NUC505_USBD_EP_REG(i, EPTXCNT) = ep_in_buffer_size[i];
						ep_in_buffer_size[i] = 0;
					}
				}
			}
		}
