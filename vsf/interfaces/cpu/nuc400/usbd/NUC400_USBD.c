...
static volatile uint16_t vsfhal_outrdy = 0, vsfhal_outen = 0;
...
{
	NUC400_USBD_EP_REG(ep, EPRSPCTL) =
				USB_EP_RSPCTL_FLUSH | USB_EP_RSPCTL_MODE_MANUAL;
	NUC400_USBD_EP_REG(ep, EPCFG) &= ~USB_EP_CFG_TYPE_MASK;
	NUC400_USBD_EP_REG(ep, EPCFG) |= type | USB_EP_CFG_VALID;
}

vsf_err_t vsfhal_usbd_ep_set_type(uint8_t idx, enum interface_usbd_eptype_t type)
{
	int8_t index_in = vsfhal_usbd_ep(idx | NUC400_USBD_EPIN);
	int8_t index_out = vsfhal_usbd_ep(idx | NUC400_USBD_EPOUT);
	uint32_t eptype;
	...
	if (index_in > 1)
	{
		...
		NUC400_USBD_EP_REG(index_in, EPINTEN) = USBD_EPINTEN_TXPKIEN_Msk;
	}
	if (index_out > 1)
	{
		index_out -= 2;
...
{
	int8_t index = vsfhal_usbd_get_free_ep(idx | NUC400_USBD_EPIN);
	if (index < 0)
	{
		return VSFERR_FAIL;
	}
	...
	else if (index > 1)
	{
		...
		if (index_out < 0)
		{
			...
			EP_Cfg_Ptr -= epsize & 1 ? epsize + 1 : epsize;
			NUC400_USBD_EP_REG(index, EPBUFSTART) = EP_Cfg_Ptr;
			NUC400_USBD_EP_REG(index, EPBUFEND) = EP_Cfg_Ptr + epsize - 1;
		}
		else if (index_out > 1)
		{
			index_out -= 2;
			NUC400_USBD_EP_REG(index, EPBUFSTART) =\
									NUC400_USBD_EP_REG(index_out, EPBUFSTART);
			NUC400_USBD_EP_REG(index, EPBUFEND) =\
									NUC400_USBD_EP_REG(index_out, EPBUFEND);
		}
		else
		{
			return VSFERR_BUG;
		}
		NUC400_USBD_EP_REG(index, EPMPS) = epsize;
		NUC400_USBD_EP_REG(index, EPCFG) |= USB_EP_CFG_DIR_IN;
		NUC400_USBD_EP_REG(index, EPCFG) &= ~(0xF << 4);
		NUC400_USBD_EP_REG(index, EPCFG) |= (idx & 0x0F) << 4;
		return VSFERR_NONE;
	}
	return VSFERR_BUG;
}
...
{
	int8_t index = vsfhal_usbd_ep(idx | NUC400_USBD_EPIN);
	...
	{
		index -= 2;
		return NUC400_USBD_EP_REG(index, EPMPS);
	}
	return 0;
}
...
vsf_err_t vsfhal_usbd_ep_set_IN_stall(uint8_t idx)
{
	...
	{
		index -= 2;
		NUC400_USBD_EP_REG(index, EPRSPCTL) =
			(NUC400_USBD_EP_REG(index, EPRSPCTL) & 0xF7) | USB_EP_RSPCTL_HALT;
		NUC400_USBD_EP_REG(index, EPRSPCTL) |= USB_EP_RSPCTL_FLUSH;
		return VSFERR_NONE;
	}
	return VSFERR_BUG;
}
...
vsf_err_t vsfhal_usbd_ep_clear_IN_stall(uint8_t idx)
{
	...
	else if (index > 1)
	{
		index -= 2;
		NUC400_USBD_EP_REG(index, EPRSPCTL) |= USB_EP_RSPCTL_FLUSH;
		NUC400_USBD_EP_REG(index, EPRSPCTL) &= ~USB_EP_RSPCTL_HALT;
		NUC400_USBD_EP_REG(index, EPRSPCTL) |= USB_EP_RSPCTL_TOGGLE;
		return VSFERR_NONE;
	}
	return VSFERR_BUG;
}
...
bool vsfhal_usbd_ep_is_IN_stall(uint8_t idx)
{
	...
	{
		index -= 2;
		return (NUC400_USBD_EP_REG(index, EPRSPCTL) & USB_EP_RSPCTL_HALT) > 0;
	}
	return true;
}
...
vsf_err_t vsfhal_usbd_ep_set_IN_count(uint8_t idx, uint16_t size)
{
	...
	{
		index -= 2;
		NUC400_USBD_EP_REG(index, EPTXCNT) = size;
		return VSFERR_NONE;
	}
	return VSFERR_BUG;
}
											...
											uint16_t size)
{
	...
	else if (index > 1)
	{
		...
		for (i = 0; i < size; i++)
		{
			NUC400_USBD_EP_REG(index, EPDAT_BYTE) = buffer[i];
		}
		return VSFERR_NONE;
	}
	return VSFERR_BUG;
...
{
	int8_t index = vsfhal_usbd_get_free_ep(idx | NUC400_USBD_EPOUT);
	if (index < 0)
	{
		return VSFERR_FAIL;
	}
	...
	else if (index > 1)
	{
		...
		else if (index_in > 1)
		{
			index_in -= 2;
			NUC400_USBD_EP_REG(index, EPBUFSTART) =\
									NUC400_USBD_EP_REG(index_in, EPBUFSTART);
			NUC400_USBD_EP_REG(index, EPBUFEND) =\
									NUC400_USBD_EP_REG(index_in, EPBUFEND);
		}
		else
		{
			return VSFERR_BUG;
...
{
	int8_t index = vsfhal_usbd_ep(idx | NUC400_USBD_EPOUT);
	if ((index < 0) || (index >= NUC400_USBD_EP_NUM))
	{
		return 0;
	}
...
uint16_t vsfhal_usbd_ep_get_OUT_count(uint8_t idx)
{
	...
	{
		index -= 2;
		return NUC400_USBD_EP_REG(index, EPDATCNT) & 0xFF;
	}
	return 0;
}
											...
											uint16_t size)
{
	...
	{
		index -= 2;
		size = min(size, NUC400_USBD_EP_REG(index, EPMPS));
		for (i = 0; i < size; i++)
		{
			buffer[i] = NUC400_USBD_EP_REG(index, EPDAT_BYTE);
		}
		vsfhal_outrdy &= ~(1 << idx);
		NUC400_USBD_EP_REG(index, EPINTSTS) = USBD_EPINTSTS_RXPKIF_Msk;
		return VSFERR_NONE;
	}
	return VSFERR_BUG;
}
...
vsf_err_t vsfhal_usbd_ep_enable_OUT(uint8_t idx)
{
	...
	{
		index -= 2;
		if (vsfhal_outrdy & (1 << idx))
		{
			if (vsfhal_usbd_callback.on_out != NULL)
			{
				vsfhal_usbd_callback.on_out(vsfhal_usbd_callback.param, idx);
			}
		}
		else
		{
			vsfhal_outen |= 1 << idx;
			NUC400_USBD_EP_REG(index, EPINTEN) |= USBD_EPINTEN_RXPKIEN_Msk;
		}
		return VSFERR_NONE;
	}
	return VSFERR_BUG;
...
void USB_Istr(void)
{
	...
	if (IrqStL & USBD_GINTSTS_USBIF_Msk) {
		...
		if (IrqSt & USBD_BUSINTSTS_RSTIF_Msk) {
			...
			{
				NUC400_USBD_EP_REG(i, EPRSPCTL) = USBD_EPRSPCTL_FLUSH_Msk;
				NUC400_USBD_EP_REG(i, EPINTSTS) = 0x1FFF;
				NUC400_USBD_EP_REG(i, EPINTEN) = 0;
				NUC400_USBD_EP_REG(i, EPCFG) = 0;
			}
		}
	}
	...
	if (IrqStL & USBD_GINTSTS_CEPIF_Msk) {
		...
		if (IrqSt & USBD_CEPINTSTS_RXPKIF_Msk) {
			...
			vsfhal_outrdy |= 1;
			if ((vsfhal_outen & 1) && (vsfhal_usbd_callback.on_out != NULL))
			{
				vsfhal_outen &= ~1;
				vsfhal_usbd_callback.on_out(vsfhal_usbd_callback.param, 0);
			}
	...
	if (IrqStL & (~3))
	{
		...
		for (i = 0; i < NUC400_USBD_EP_NUM - 2; i++)
		{
			...
			if (IrqStL & (1 << (i + 2)))
			{
				IrqSt = NUC400_USBD_EP_REG(i, EPINTSTS);
				IrqSt &= NUC400_USBD_EP_REG(i, EPINTEN);

				if (IrqSt & USBD_EPINTSTS_TXPKIF_Msk)
				{
					NUC400_USBD_EP_REG(i, EPINTSTS) = USBD_EPINTSTS_TXPKIF_Msk;
					if (vsfhal_usbd_callback.on_in != NULL)
					{
				...
				{
					NUC400_USBD_EP_REG(i, EPINTEN) &= ~USBD_EPINTEN_RXPKIEN_Msk;

					vsfhal_outrdy |= 1 << ep;
					if ((vsfhal_outen & (1 << ep)) &&
						(vsfhal_usbd_callback.on_out != NULL))
					{
						vsfhal_outen &= ~(1 << ep);
